...
import { Business, Category, FormatListNumbered, SquareFoot, LineWeight, Layers, ViewList } from '@mui/icons-material'
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8">
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl sm:text-2xl font-bold text-gray-900 mb-2">Add New Product</h1>
            </div>

            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {fields.filter(field => permissions[field.key]).map(field => {
                    ...
                    <div key={field.key} className="space-y-2">
                      {isAuto ? (
                        <Autocomplete
                          ...
                          value={value}
                          onChange={(_, newVal) => {
                            if (typeof newVal === 'string' && newVal.startsWith('Add "')) {
                          ...
                          onInputChange={(_, newInputValue) => {
                            setInputValues(prev => ({ ...prev, [field.key]: newInputValue }))
                          }}
            ...
            {/* Add New Dialog */}
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="xs" PaperProps={{
              ...
              <DialogActions>
                <Button onClick={() => setDialogOpen(false)} variant="outlined" color="secondary">Cancel</Button>
                <Button onClick={handleDialogAdd} variant="contained" disabled={!dialogValue.trim()} sx={{ background: 'linear-gradient(to right, #6366f1, #a21caf)' }}>
                  Add
                </Button>
              </DialogActions>
            </Dialog>

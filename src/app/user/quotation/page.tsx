...
const QuotationPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const products: Product[] = Array.isArray(res.data?.data)
          ? res.data.data
          : [];
        ...
        const productGroups: ProductGroup[] = Object.entries(grouped).map(
          ([name, products]) => ({
            name,
  ...
  useEffect(() => {
    const gradeFieldsStr =
      typeof window !== "undefined"
        ? localStorage.getItem("quotation_grade_fields")
        : null;
    const productFieldsStr =
      typeof window !== "undefined"
        ? localStorage.getItem("quotation_product_fields")
        : null;
    let gradeFields: any = {};
  ...
  const handlePricingChange = useCallback(
    (productId: number, field: keyof ProductPricing[number]) =>
  ...
  const generateModernPDF = useCallback(() => {
    ...
    const getColumnWidth = (columnKey: string, length: number) => {
      ...
      const totalBaseWidth = visibleColumns.reduce(
        (sum, col) => sum + (baseWidths[col.key] || 20),
        0
      );
      ...
      <Card sx={{ maxWidth: 1200, mx: "auto", boxShadow: 3 }}>
        ...
        <CardContent sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
          ...
          <Box sx={{ mt: 4, mb: 3 }}>
            ...
            <Box
              component="ol"
              sx={{ pl: { xs: 2, sm: 3 }, "& li": { mb: { xs: 2, sm: 1 } } }}
            >
              <Box component="li">
                <Box
                  sx={{
